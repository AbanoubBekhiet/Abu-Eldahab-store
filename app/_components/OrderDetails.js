...
export default async function OrderDetails({ orders, params }) {
	const selectedOrder = orders?.find((order) => order.id == params?.order_id);

	if (!params?.order_id || !selectedOrder) {
		return (
	...
	return (
		<Table dir="rtl">
			<TableCaption>شكراً لطلبك من شركة أبو الدهب</TableCaption>
			<TableHeader>
				<TableRow>
