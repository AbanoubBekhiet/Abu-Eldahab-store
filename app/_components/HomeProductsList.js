...
function HomeProductsList({ category, user }) {
    ...
    const handleScrollNavigation = (direction) => {
        ...
        const newScrollGoal = direction === "left" 
        ...
        const remainingSpace = container.scrollWidth - (Math.abs(newScrollGoal) + container.clientWidth);

        if (direction === "right" && remainingSpace < triggerThreshold && hasMore && !loading) {
    ...
    return (
        <section className="my-12">
            <header className="flex items-center justify-between p-4 mb-6 bg-[var(--color-three)] rounded-xl">
                <h3 className="text-[var(--color-one)] text-xl font-bold">{category.name}</h3>
                <Link href={`/products?filter=${category.name}&search=لا+يوجد`}>
                    <Button variant="outline" className="text-[var(--color-one)] gap-2 h-9 text-sm">
            ...
            <div className="relative group w-full">
                ...
                >
                    {products.map((product) => (
                        <div key={product.id} className="min-w-[160px] sm:min-w-[200px] max-w-[280px] sm:max-w-[320px] flex-shrink-0">
                    ...
                    {loading && (
                        <div className="flex gap-3">
                            {[...Array(4)].map((_, i) => (
                                <div key={i} className="min-w-[160px] sm:min-w-[200px] h-[280px] bg-gray-100 animate-pulse rounded-lg" />
                            ))}
                        </div>
                    )}
                </div>
